const ICON_PATHS = {
  ...
  testTube: '<path d="M9 3h6"/><path d="M10 9V3"/><path d="M14 9V3"/><path d="M9 9a3 3 0 0 0 6 0v0a5 5 0 0 1 1 3c0 3.5-2.5 6-4 7.5-.5.5-1 .5-1.5 0C9 18 8 15.5 8 12a5 5 0 0 1 1-3Z"/>',
  eye: '<path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/>',
  scale: '<path d="m16 16 3-8 3 8c0 1.1-.9 2-2 2h-2c-1.1 0-2-.9-2-2Z"/><path d="m2 16 3-8 3 8c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2Z"/><path d="M5 8h12"/><path d="M11 8v8"/>'
};

function buildSvg(name, color, size) {
  var pathData = ICON_PATHS[name];
  if (!pathData) return '';
  var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="' + color + '" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">' + pathData + '</svg>';
...
Component({
  ...
  observers: {
    'name, color, size': function (name, color, size) {
      this._updateIcon(name, color, size);
    }
  },

  lifetimes: {
    attached: function () {
      this._updateIcon(this.data.name, this.data.color, this.data.size);
    }
  },

  methods: {
    _updateIcon: function (name, color, size) {
      var src = buildSvg(name, color, size);
      var iconSize = size || 48;
      if (src !== this.data.iconSrc || iconSize !== this.data.iconSize) {
        this.setData({ iconSrc: src, iconSize: iconSize });
      }
    }
  }
});
